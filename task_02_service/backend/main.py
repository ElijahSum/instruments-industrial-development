...
DEFAULT_SEED_PATH = BACKEND_DIR / "RU_Electricity_Market_PZ_dayahead_price_volume.csv"
...
def _normalize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    missing_columns = [column for column in DATA_COLUMNS if column not in df.columns]
    if missing_columns:
    ...
    normalized["id"] = pd.to_numeric(normalized["id"], errors="raise").astype("int64")
    ...
    normalized = normalized.sort_values(["timestep", "id"]).reset_index(drop=True)
    normalized["timestep"] = normalized["timestep"].dt.strftime(DATETIME_FORMAT)
...
def _ensure_storage_ready() -> None:
    with WRITE_LOCK:
        ...
        if not source_path.exists():
            raise ValueError(
                "No source CSV found. Expected backend/data.csv or the seed CSV file."
            )
...
def delete_record(record_id: int) -> dict[str, int]:
    try:
        with WRITE_LOCK:
            ...
            updated = dataframe[dataframe["id"] != record_id].reset_index(drop=True)
            _write_dataframe(updated)
    except HTTPException:
        raise
    except ValueError as exc:
